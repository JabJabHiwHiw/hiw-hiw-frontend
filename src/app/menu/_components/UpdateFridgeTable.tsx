...
export default function UpdateFridgeTable({ mid }: { mid: string }) {
  ...
  const [menuIngredients, setMenuIngredients] = useState<Ingredient[] | null>(
    null
  )
  const [fridgeResponse, setFridgeResponse] = useState<FridgeItem[] | null>(
    null
  )
  ...
  const handleUpdate = async () => {
    const incompleteEntries = tableData.some(
      (row) => !row.deleteMode && row.remaining_amount.trim() === ''
    )

    if (incompleteEntries) {
      setErrorMessage(
        'Please fill in the remaining amount for all ingredients before updating.'
      )
      return
    }
  ...
  return (
    <div className="overflow-x-auto space-y-6 w-full">
      ...
      {tableData.length > 0 ? (
          ...
          <table className="min-w-full bg-white">
            <thead>
              <tr className="bg-primary-200 border border-primary-300">
                ...
                <th className="py-2 px-4 text-left h4 font-bold">
                  In Fridge Amount
                </th>
                <th className="py-2 px-4 text-left h4 font-bold">
                  Remaining Amount
        ...
        <div className="h4 font-bold text-enterm text-center">
